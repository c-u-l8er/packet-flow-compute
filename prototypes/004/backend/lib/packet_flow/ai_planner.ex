defmodule PacketFlow.AIPlanner do
  ...
  defp build_planning_prompt(intent, capabilities, context) do
    ...
    command_type = Map.get(context, :command_type, :general)

    # Get command-specific guidance
    command_guidance = get_command_guidance(command_type)
    ...
    defp get_command_guidance(:analyze) do
    """
    ANALYZE MODE: Your goal is to provide ANALYSIS, not summarization.

    Focus on:
    - Finding patterns, trends, and insights
    - Identifying relationships and correlations
    - Detecting anomalies or unusual aspects
    - Providing deep understanding and interpretation
    - Explaining WHY things happen, not just WHAT happened
    - Offering data-driven insights and conclusions

        MANDATORY: You MUST use the capability:
    - deep_conversation_analysis: This is the ONLY correct capability for analyze commands

    NEVER use these for analyze commands:
    - analyze_conversation: WRONG - This is a legacy capability with basic analysis only
    - conversation_summarization: WRONG - This is for summaries, not analysis
    - create_room_summary: WRONG - This creates activity stats, not deep analysis

    The user specifically requested ANALYSIS, not summaries or activity stats.
    """
  end

    defp get_command_guidance(:summarize) do
    """
    SUMMARIZE MODE: Your goal is to provide SUMMARIES, not analysis.

    Focus on:
    - Condensing information into key points
    - Highlighting main topics and themes
    - Providing concise overviews
    - Extracting essential information
    - Creating digestible summaries of complex data
    - Focusing on WHAT happened, not why

        MANDATORY: You MUST use the capability:
    - conversation_summarization: This is the ONLY correct capability for summarize commands

    NEVER use these for summarize commands:
    - analyze_conversation: WRONG - This does analysis, not summarization
    - deep_conversation_analysis: WRONG - This is for analysis, not summaries
    - create_room_summary: WRONG - This creates activity stats, not content summaries

    The user specifically requested a SUMMARY, not analysis or activity stats.
    """
  end

  defp get_command_guidance(_other) do
    """
    GENERAL MODE: Use your best judgment to determine the most appropriate approach.
    """
  end
  ...
  defp validate_plan(plan, available_capabilities) do
    ...
    # Handle both atom and string keys, and ensure execution_plan and steps exist
    ...
    |> Enum.map(fn step ->
      capability_id = step["capability_id"] || step[:capability_id]
    ...
    # Update the plan with validated steps, preserving the original key structure
    updated_execution_plan = Map.put(execution_plan, :steps, validated_steps)
    Map.put(plan, :execution_plan, updated_execution_plan)
  end

  defp get_capability_ids(capabilities) when is_list(capabilities) do
    Enum.map(capabilities, fn cap -> to_string(cap.id) end)
  end

  defp get_capability_ids(_), do: []

  defp find_similar_capability(target_id, capabilities) when is_list(capabilities) do
  ...
  defp find_similar_capability(_, _), do: nil
end
