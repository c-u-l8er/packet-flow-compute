defmodule PacketFlow.Capabilities.SimpleChatCapabilities do
  ...
  def __capabilities__ do
    [
      ...
      #   intent: "Basic conversation analysis for general purposes (legacy capability)",
      ...
      # },
      %{
        id: :deep_conversation_analysis,
        intent: "ANALYZE command: Perform deep analysis of conversation patterns, relationships, behavioral insights, and anomaly detection",
        requires: [:room_id, :analysis_scope],
        provides: [:pattern_analysis, :relationship_insights, :behavioral_patterns, :anomaly_detection],
        effects: []
      },
      %{
        id: :conversation_summarization,
        intent: "SUMMARIZE command: Generate concise summaries with key points, main topics, decisions, and action items",
        requires: [:room_id, :summary_format],
        provides: [:key_points, :main_topics, :decisions_made, :action_items],
        effects: []
      }
    ]
  end
  ...
  def send_message(payload, context) do
    try do
      ...
      # Check if this is being called from an AI execution context
      # In that case, return the message content instead of sending it
      # to avoid duplicate messages (the room channel will handle sending)
      if context[:execution_id] do
        content = payload["content"] || payload[:content]
  ...
  def analyze_conversation(payload, context) do
    try do
      # Handle both string and atom keys
      room_id = payload["room_id"] || payload[:room_id]
      raw_message_count = payload["message_count"] || payload[:message_count] || 50

      # Handle "all" parameter by converting to a large number, or parse integer
      message_count = case raw_message_count do
        "all" -> 1000  # Use a large number to get all messages
        :all -> 1000
        count when is_binary(count) ->
          case Integer.parse(count) do
            {num, _} -> num
            :error -> 50  # Default fallback
          end
  ...
  def deep_conversation_analysis(payload, context) do
    try do
      room_id = payload["room_id"] || payload[:room_id]
      analysis_scope = payload["analysis_scope"] || payload[:analysis_scope] || "comprehensive"
      message_count = payload["message_count"] || payload[:message_count] || 100

      messages = PacketflowChat.Chat.list_room_messages(room_id, message_count)
      Logger.info("deep_conversation_analysis: Analyzing #{length(messages)} messages for room #{room_id}")

      case perform_deep_analysis_with_llm(messages, analysis_scope, context) do
        {:ok, llm_analysis} ->
          {:ok, llm_analysis}

        {:error, :no_api_keys_configured} ->
          {:ok, %{
            message: "💡 **Deep Analysis Available with API Key**\n\nTo get advanced conversation analysis, please set up an API key:\n\n• Set `ANTHROPIC_API_KEY` environment variable for Claude\n• Or set `OPENAI_API_KEY` environment variable for GPT-4\n\nFor now, here's a basic pattern analysis:",
            pattern_analysis: analyze_patterns_basic(messages),
            source: "basic_with_setup_info"
          }}
        ...
        {:error, _llm_error} ->
          {:ok, %{
            pattern_analysis: analyze_patterns_basic(messages),
            relationship_insights: analyze_relationships_basic(messages),
            behavioral_patterns: analyze_behavior_basic(messages),
            source: "basic_fallback"
          }}
      end
    rescue
      error ->
        Logger.error("deep_conversation_analysis capability failed: #{inspect(error)}")
        {:error, {:execution_error, error}}
    end
  end

  def conversation_summarization(payload, context) do
    try do
      room_id = payload["room_id"] || payload[:room_id]
      raw_summary_format = payload["summary_format"] || payload[:summary_format] || "comprehensive"

      # Handle summary_format being passed as a map from execution plans
      summary_format = case raw_summary_format do
        %{} = format_map ->
          # Extract focus or use comprehensive as default
          Map.get(format_map, "focus", "comprehensive")
      ...
      messages = PacketflowChat.Chat.list_room_messages(room_id, message_count)
      Logger.info("conversation_summarization: Summarizing #{length(messages)} messages for room #{room_id}")

      case perform_summarization_with_llm(messages, summary_format, context) do
        {:ok, llm_summary} ->
          {:ok, llm_summary}

        {:error, :no_api_keys_configured} ->
          {:ok, %{
            message: "💡 **AI Summarization Available with API Key**\n\nTo get intelligent conversation summaries, please set up an API key:\n\n• Set `ANTHROPIC_API_KEY` environment variable for Claude\n• Or set `OPENAI_API_KEY` environment variable for GPT-4\n\nFor now, here's a basic summary:",
            key_points: extract_key_points_basic(messages),
            main_topics: extract_topics_basic(messages),
            source: "basic_with_setup_info"
          }}
        ...
        {:error, _llm_error} ->
          {:ok, %{
            key_points: extract_key_points_basic(messages),
            main_topics: extract_topics_basic(messages),
            decisions_made: extract_decisions_basic(messages),
            action_items: extract_actions_basic(messages),
            source: "basic_fallback"
          }}
      end
    rescue
      error ->
        Logger.error("conversation_summarization capability failed: #{inspect(error)}")
        {:error, {:execution_error, error}}
    end
  end
  ...
  defp perform_deep_analysis_with_llm(messages, analysis_scope, context) do
    prompt = build_deep_analysis_prompt(messages, analysis_scope)
    call_llm_for_analysis(prompt, context)
  end

  defp build_deep_analysis_prompt(messages, analysis_scope) do
    message_text = format_messages_for_analysis(messages)

    """
    Perform deep analysis of this conversation. Focus on providing ANALYSIS, not summarization.
    ...
    1. Communication patterns and trends
    2. Relationship dynamics between participants
    3. Behavioral patterns and engagement levels
    4. Anomalies or notable changes in conversation flow
    5. Sentiment evolution and emotional patterns
    6. Topic depth and complexity analysis
    7. Decision-making processes and influence patterns

    Respond in JSON format:
    {
      "pattern_analysis": {
        "communication_frequency": "description",
        "response_times": "analysis",
        "participation_patterns": "insights"
      },
      "relationship_insights": {
        "interaction_quality": "assessment",
        "collaboration_patterns": "observations",
        "conflict_resolution": "analysis"
      },
      "behavioral_patterns": {
        "engagement_levels": "analysis",
        "communication_styles": "assessment",
        "topic_leadership": "observations"
      },
      "anomaly_detection": {
        "unusual_patterns": ["list of anomalies"],
        "conversation_breaks": "analysis",
        "behavioral_changes": "insights"
      }
    }
    """
  end
  ...
  defp perform_summarization_with_llm(messages, summary_format, context) do
    prompt = build_summarization_prompt(messages, summary_format)
    call_llm_for_analysis(prompt, context)
  end

  defp build_summarization_prompt(messages, summary_format) do
    message_text = format_messages_for_analysis(messages)

    """
    Create a concise summary of this conversation. Focus on SUMMARIZING key information, not analyzing patterns.
    ...
    4. Notable information or facts mentioned

    Respond in JSON format:
    {
      "key_points": ["list of main points discussed"],
      "main_topics": ["primary conversation topics"],
      "decisions_made": ["decisions reached during conversation"],
      "action_items": ["tasks or actions mentioned"],
      "participants_mentioned": ["key people involved"],
      "timeline_summary": "chronological overview if relevant"
    }
    """
  end

  # Basic Analysis Fallback Functions
  defp analyze_patterns_basic(messages) do
    %{
      message_frequency: calculate_message_frequency_new(messages),
      active_periods: identify_active_periods(messages),
      response_patterns: analyze_response_patterns(messages)
    }
  end

  defp analyze_relationships_basic(messages) do
    %{
      participant_interactions: count_participant_interactions(messages),
      conversation_initiators: identify_conversation_starters(messages)
    }
  end

  defp analyze_behavior_basic(messages) do
    %{
      message_lengths: analyze_message_lengths(messages),
      communication_styles: basic_communication_styles(messages)
    }
  end

  defp extract_key_points_basic(messages) do
    messages
    |> Enum.take(10)
    |> Enum.map(&("• #{String.slice(&1.content, 0, 100)}#{if String.length(&1.content) > 100, do: "...", else: ""}"))
  end

  defp extract_topics_basic(messages) do
    all_content = messages |> Enum.map(& &1.content) |> Enum.join(" ")
    extract_keywords(all_content) |> Enum.take(5)
  end

  defp extract_decisions_basic(messages) do
    messages
    |> Enum.filter(&(String.contains?(&1.content, ["decided", "agree", "will do", "let's"])))
    |> Enum.map(&("• #{&1.content}"))
    |> Enum.take(5)
  end

  defp extract_actions_basic(messages) do
    messages
    |> Enum.filter(&(String.contains?(&1.content, ["todo", "action", "next step", "will", "should"])))
    |> Enum.map(&("• #{&1.content}"))
    |> Enum.take(5)
  end

  # Helper functions for basic analysis
  defp calculate_message_frequency_new(messages) do
    if length(messages) > 1 do
      first_time = List.first(messages).created_at
      last_time = List.last(messages).created_at
      duration_minutes = DateTime.diff(last_time, first_time, :minute)
      if duration_minutes > 0, do: length(messages) / duration_minutes, else: 0
    else
      0
    end
  end

  defp identify_active_periods(messages) do
    messages
    |> Enum.group_by(&DateTime.to_date(&1.created_at))
    |> Enum.map(fn {date, day_messages} ->
      %{date: date, message_count: length(day_messages)}
    end)
  end

  defp analyze_response_patterns(messages) do
    %{
      average_response_time: "Not calculated in basic mode",
      conversation_threads: count_conversation_threads(messages)
    }
  end

  defp count_participant_interactions(messages) do
    messages
    |> Enum.group_by(& &1.user_id)
    |> Enum.map(fn {user_id, user_messages} ->
      %{user_id: user_id, message_count: length(user_messages)}
    end)
  end

  defp identify_conversation_starters(messages) do
    messages
    |> Enum.group_by(& &1.user_id)
    |> Enum.map(fn {user_id, user_messages} ->
      first_message = Enum.min_by(user_messages, & &1.created_at)
      %{user_id: user_id, first_message_time: first_message.created_at}
    end)
  end

  defp analyze_message_lengths(messages) do
    lengths = messages |> Enum.map(&String.length(&1.content))
    %{
      average_length: if(length(lengths) > 0, do: Enum.sum(lengths) / length(lengths), else: 0),
      shortest: if(length(lengths) > 0, do: Enum.min(lengths), else: 0),
      longest: if(length(lengths) > 0, do: Enum.max(lengths), else: 0)
    }
  end

  defp basic_communication_styles(messages) do
    question_count = messages |> Enum.count(&String.contains?(&1.content, "?"))
    exclamation_count = messages |> Enum.count(&String.contains?(&1.content, "!"))

    %{
      questions_asked: question_count,
      exclamations_used: exclamation_count,
      total_messages: length(messages)
    }
  end

  defp count_conversation_threads(_messages) do
    # Simplified thread counting for basic mode
    1
  end

  defp format_messages_for_analysis(messages) do
    messages
    |> Enum.take(50)  # Limit to last 50 messages for LLM processing
    |> Enum.map(fn msg ->
      timestamp = DateTime.to_string(msg.created_at)
      "#{timestamp} - User #{msg.user_id}: #{msg.content}"
    end)
    |> Enum.join("\n")
  end

  defp call_llm_for_analysis(prompt, _context) do
    # Call LLM directly without using analyze_conversation_with_llm
    case call_llm_with_config(prompt) do
      {:ok, llm_response} ->
        # Try to parse as JSON, fallback to plain text
        case Jason.decode(llm_response) do
          {:ok, parsed_json} -> {:ok, parsed_json}
          {:error, _} -> {:ok, %{"summary" => llm_response}}
        end
