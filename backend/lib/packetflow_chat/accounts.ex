defmodule PacketflowChat.Accounts do
  ...
  def update_user_email(user, token) do
    ...
    with {:ok, query} <- UserToken.verify_change_email_token_query(token, context),
         ...
         {:ok, _} <- Repo.transaction(update_user_email_multi(user, email, context)) do
      :ok
    else
      _ -> :error
    end
  ...
  defp update_user_email_multi(user, email, context) do
    changeset =
      user
  ...
  def update_user_password(user, password, attrs) do
    ...
    changeset = if User.valid_password?(user, password) do
      changeset
    else
      Ecto.Changeset.add_error(changeset, :current_password, "is not valid")
    end
  ...
  def deliver_user_reset_password_instructions(%User{} = user, reset_password_url_fun)
    ...
    {:ok, %{to: user.email, body: "Reset password token: #{encoded_token}"}}
  end
